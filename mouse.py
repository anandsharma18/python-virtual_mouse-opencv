...
def start_mouse():
    ...
    lower = np.array([v[0], v[1], v[2]])  # HSV lower
    upper = np.array([v[3], v[4], v[5]])  # HSV upper
    ...
    while True:
        ...
        _, contours, h, = cv2.findContours(thresh.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
        ...
        if len(contours) >= 2:
            ...
            if not flags[0]:
                if np.any(abs(center[0] - old_center[0]) > 5):
                    Thread(target=gui.moveTo, args=(mposx + (center[0][0] - old_center[0][0]) * sx, mposy +
                                                    (center[0][1] - old_center[0][1]) * sy, 0.1,
                                                    gui.easeInOutQuad)).start()
            ...
            if error < 40:
                ...
                if finger_frame_count[1] > 20:
                    ...
                    if np.any(abs(center[1] - old_center[1]) > 5):
                        Thread(target=gui.moveTo, args=(mposx + (center[1][0] - old_center[1][0]) * sx, mposy +
                                                        (center[1][1] - old_center[1][1]) * sy, 0.1,
                                                        gui.easeInOutQuad)).start()
            ...
            if mouse_down:
                Thread(target=gui.mouseUp, args=()).start()
                _thread.start_new_thread(gui.mouseUp, ())
                mouse_down = False
            flags = [True, False, False]
